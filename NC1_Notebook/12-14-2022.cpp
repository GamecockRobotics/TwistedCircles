...
//Declaring variables
int catapultTarget = 0, counter = 0;
bool catapultFlag = true, intakeFlag = true;
...
void initialize() {
	...
	catapult_L.set_brake_mode(MOTOR_BRAKE_COAST);
	catapult_R.set_brake_mode(MOTOR_BRAKE_COAST);

	catapult_L.set_encoder_units(pros::E_MOTOR_ENCODER_DEGREES);
}
...
void shoot(){
	...
	if (catapultTarget == 0 && catapultFlag) {
		catapultTarget = 127;
	} else if (catapultTarget == 127 && catapult_L.get_efficiency() < 5) {
		pros::delay(1000);
		catapultTarget = 0;
		//catapult_L.move(catapultTarget);
		//catapult_R.move(catapultTarget);
		catapultFlag = false;
		catapult_L.tare_position();
	} else if (catapultTarget == 0 && !catapultFlag){
		catapultTarget = -127;
		catapult_L.move(catapultTarget);
		catapult_R.move(catapultTarget);
		pros::delay(500);
		//catapult_L.move_absolute(80, catapultTarget);
		//catapult_R.move_absolute(80, catapultTarget);
		// while (!((catapult_L.get_position() < -75) && (catapult_L.get_position() > -85))) {
	...
	//pros::screen::print(pros::E_TEXT_MEDIUM, 300, 204, std::to_string(counter));
