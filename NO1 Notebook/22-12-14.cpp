...
void opcontrol() {
	...
	while (true) {
		...
		mm_forward = (track_forward - prev_track_forward)*track_wheel_size;
		...
		x_loc += cos(theta)*x_local - sin(theta)*y_local;
		y_loc += sin(theta)*x_local + cos(theta)*y_local;
        ...
        // Debugging values printed to brain
		pros::lcd::set_text(0, std::to_string(x_loc));
		pros::lcd::set_text(1, std::to_string(cos(y_loc)));
		pros::lcd::set_text(2, std::to_string(theta));
		//pros::lcd::set_text(3, std::to_string());	
		pros::lcd::set_text(4, std::to_string(x_local));
		pros::lcd::set_text(5, std::to_string(y_local));
		pros::lcd::set_text(6, std::to_string(delta_theta));
